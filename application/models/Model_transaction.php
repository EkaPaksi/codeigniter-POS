...
class Model_transaction extends CI_Model
{
	...
	{
		$query = $this->db->select('sum(price) as total from transactions where day(created_at)=' . date('d') . '');
	...
	{
		$query = $this->db->select('sum(price) as total from transactions where month(created_at)=' . date('m') . '');
		return $query->get()->result();
		// return $query->row()->total;
	}
}
